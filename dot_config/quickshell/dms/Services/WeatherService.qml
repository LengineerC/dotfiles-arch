...
Singleton {
    ...
    readonly property var curlBaseCmd: ["curl", "-sS", "--fail", "--connect-timeout", "8", "--max-time", "20", "--limit-rate", "100k", "--compressed"]
	...
	const configuredName = SessionData.isGreeterMode ? GreetdSettings.weatherLocation : SettingsData.weatherLocation;
        if (configuredName) {
            root.location = {
                city: configuredName,
                country: "",
	...
	const weatherCmd = lowPriorityCmd.concat(["curl", "-sS", "--fail", "--connect-timeout", "8", "--max-time", "20", "--limit-rate", "150k", "--compressed"]);
